...
var simpleneuron = (function () {
    ...
    function Network(layersizes, options) {
        var size = 0;
        var outputoffset = 0;
        var neurons = [];
		var layeroffsets = [ 0 ];
		...
		var ninputs = layersizes[0];
        
        for (var k = 0; k < nlayers; k++)
            size += layersizes[k];
		for (var k = 1; k < nlayers; k++)
			layeroffsets[k] = layeroffsets[k - 1] + layersizes[k - 1];
		
        outputoffset = size - layersizes[nlayers - 1];
        ...
        for (var k = 1; k < layersizes.length; k++)
            connectLayers(k - 1, k);
        ...
        this.train = function (inputs, expected, learningrate) {
            ...
            for (var nl = layersizes.length; nl-- > 0;) {
                var from = layeroffsets[nl];
                var to = from + layersizes[nl];
                
                for (var k = from; k < to; k++)
                    if (nl === layersizes.length - 1) {
                        errors[k] = (expected[k - from] - outputs[k - from]);
                    }
                    else {
                        var error = 0;
                        var to2 = to + layersizes[nl + 1];
        ...
        function connectLayers(n, m) {
            ...
            var inputs = neurons.slice(k0, j0);
            
            for (var j = 0; j < layersizes[m]; j++) {
                neurons[j + j0].inputs(inputs);
                neurons[j + j0].weights(randomWeights(inputs.length));
            }
        }
    }
